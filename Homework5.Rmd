...
eigen_type<-eigen(m)
print(eigen(m))
typeof(eigen_type$values)# complex because there are imaginary numbers
typeof(eigen_type$vectors)# complex because there are imaginary numbers
```
# Problem 2
```{r,echo=TRUE}
...
dataframe[sample(1:26,4),1]<-NA #select 4 random numbers out of the data frame and replace with NA 
print(dataframe)
...
mean((d_frame1[,1]),na.rm=TRUE)# take the mean without the NA in the data
```
